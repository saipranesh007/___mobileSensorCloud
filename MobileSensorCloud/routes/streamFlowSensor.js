...
function getSensorData(sensor){
	...
	request({
	   ...
	   checkInDatabase(sensor,finalDate,time,avg);
			...
			checkInDatabase(sensor,finalDate,time,2.1);
			console.log("no data");
		}
	});
};
...
function checkInDatabase (sensor,date,time,avgValue){
	...
	(function check(){
		...
		mongo.connect(function(err,db){
			var coll = mongo.collection('streamFlowDataCollection');
...
exports.mine = function(){
	...
	mongo.connect(function(err,db){
		...
		coll.find({sensorType:"Stream Flow", sensorStatus: true}).toArray(function(err, results){
			if(err){
