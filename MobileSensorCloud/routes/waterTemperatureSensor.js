...
function getSensorData(sensor){
	...
	request({
	   ...
	   checkInDatabase(sensor,finalDate,time,avg);
			...
			checkInDatabase(sensor,finalDate,time,60);
			console.log("no data");
		}
	});
};
...
function checkInDatabase (sensor,date,time,avgValue){
	...
	(function check(){
		...
		mongo.connect(function(err,db){
			var coll = mongo.collection('waterTemperatureDataCollection');
...
exports.mine = function(){
	...
	mongo.connect(function(err,db){
		...
		coll.find({sensorType:"Water Temperature", sensorStatus: true}).toArray(function(err, results){
			if(err){
