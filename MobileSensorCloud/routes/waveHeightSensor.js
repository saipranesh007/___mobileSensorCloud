...
function getSensorData(sensor){
	...
	request({
	   ...
	   checkInDatabase(sensor,finalDate,time,avg);
			...
			checkInDatabase(sensor,finalDate,time,11);
			console.log("no data");
		}
	});
};
...
function checkInDatabase (sensor,date,time,avgValue){
	...
	(function check(){
		...
		mongo.connect(function(err,db){
			var coll = mongo.collection('WaveHeightAndDirectionDataCollection');
...
exports.mine = function(){
	...
	mongo.connect(function(err,db){
		...
		coll.find({sensorType:"Wave Height and Direction", sensorStatus: true}).toArray(function(err, results){
			if(err){
