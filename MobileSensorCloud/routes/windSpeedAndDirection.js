...
function getSensorData(sensor){
	...
	request({
	   ...
	   checkInDatabase(sensor,finalDate,time,avg);
			...
			checkInDatabase(sensor,finalDate,time,0.10);
			console.log("no data");
		}
	});
};
...
function checkInDatabase (sensor,date,time,avgValue){
	...
	(function check(){
		...
		mongo.connect(function(err,db){
			var coll = mongo.collection('windSpeedAndDirectionDataCollection');
...
exports.mine = function(){
	...
	mongo.connect(function(err,db){
		...
		coll.find({sensorType:"Wind Speed and Direction", sensorStatus: true}).toArray(function(err, results){
			if(err){
